...
module Advent2022
  module CampCleanup
    ...
    def self.one_section_for_input(list_of_sections)
      all_sections = parse_to_sections(list_of_sections)
    ...
    sig { params(list_of_sections: String).returns(T::Array[T::Array[T::Array[Integer]]]) }
    def self.any_overlapping_section_for_input(list_of_sections)
      all_sections = parse_to_sections(list_of_sections)
      all_sections.select do |(section1, section2)|
        any_overlapping_section(T.must(section1), T.must(section2))
      end
    end

    sig { params(list_of_sections: String).returns(T::Array[T::Array[T::Array[Integer]]]) }
    def self.parse_to_sections(list_of_sections)
      list_of_sections.split("\n").map do |line|
    ...
    sig { params(inner_section: T::Array[Integer], outer_section: T::Array[Integer]).returns(T::Boolean) }
    def self.any_overlapping_section(inner_section, outer_section)
      inner_low, inner_high = inner_section
      outer_low, outer_high = outer_section
      ((inner_low..inner_high).to_a & (outer_low..outer_high).to_a).any?
    end
  end
end
