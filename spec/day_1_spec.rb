...
10000
  ...
  describe '.elf_with_most_calories' do
    it 'returns the 4th elf' do
      elves = Advent2022.elf_calories_store(example_input)
      expect(Advent2022.elf_with_most_calories(elves).id).to eq(4)
    end
  end
  ...
  describe 'for day_1_input' do
    it 'returns the 209th elf' do
      ...
      elves = Advent2022.elf_calories_store(fixture_input)
      expect(Advent2022.elf_with_most_calories(elves).id).to eq(209)
    end

    it 'returns the 74_198 total_calories for the 209th elf' do
      fixture_input = File.read('./spec/fixtures/day_1_input.txt')
      elves = Advent2022.elf_calories_store(fixture_input)
      elf = elves.find { |e| e.id == 209 }
    ...
    it 'returns top 3 elves by calorie count' do
      fixture_input = File.read('./spec/fixtures/day_1_input.txt')
      elves = Advent2022.elf_calories_store(fixture_input)
      elves_order_by_calorie_count = Advent2022.elves_order_by_calorie_count(elves)
